...
import BotdexRouter01 from '../../build/BotdexRouter01.json'
import BotdexMigrator from '../../build/BotdexMigrator.json'
import BotdexRouter from '../../build/BotdexRouter.json'
...
export async function v2Fixture( [wallet]: Wallet[], provider: Web3Provider): Promise<V2Fixture> {
  ...
  // deploy V2
  const factoryV2 = await deployContract(wallet, BotdexFactory, [wallet.address])

  // deploy routers
  const router01 = await deployContract(wallet, BotdexRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, BotdexRouter, [factoryV2.address, WETH.address], overrides)

  // event emitter for testing
  const routerEventEmitter = await deployContract(wallet, RouterEventEmitter, [])

  // deploy migrator
  const migrator = await deployContract(wallet, BotdexMigrator, [factoryV1.address, router01.address], overrides)
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IBotdexPair.abi), provider).connect(wallet)
