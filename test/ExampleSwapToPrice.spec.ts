...
const { defaultAbiCoder, formatEther } = ethers.utils
...
describe('ExampleSwapToPrice', () => {
  const provider = new MockProvider({ ganacheOptions: {
      ...
      gasLimit: 9999999,
      allowUnlimitedContractSize: true //TODO
    }
  })
  ...
  describe('#swapToPrice', () => {
    ...
    it('moves the price to 1:90', async () => {
      await expect(
        ...
        // (1e19 + 531442217054986118) : (1e21 - 50366593250622391678) ~= 1:90
        .to.emit(token0, 'Transfer')
        .withArgs(wallet.address, swapToPriceExample.address, '531442217054986118')
        .to.emit(token0, 'Approval')
        .withArgs(swapToPriceExample.address, router.address, '531442217054986118')
        .to.emit(token0, 'Transfer')
        .withArgs(swapToPriceExample.address, pair.address, '531442217054986118')
        .to.emit(token1, 'Transfer')
        .withArgs(pair.address, wallet.address, '50366593250622391678')
    })
    ...
    it('moves the price to 1:110', async () => {
      await expect(
        ...
        .to.emit(token1, 'Transfer')
        .withArgs(wallet.address, swapToPriceExample.address, '47855224842180823313')
        .to.emit(token1, 'Approval')
        .withArgs(swapToPriceExample.address, router.address, '47855224842180823313')
        .to.emit(token1, 'Transfer')
        .withArgs(swapToPriceExample.address, pair.address, '47855224842180823313')
        .to.emit(token0, 'Transfer')
        .withArgs(pair.address, wallet.address, '455825155834428295')
    })
    ...
    it('swap gas cost', async () => {
      ...
      expect(receipt.gasUsed).to.eq('163850')
